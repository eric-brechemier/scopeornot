...
// complete with Node.js require.resolve() and require.cache.
  ...
  function requireMissingNeeds(needs,context){
    ...
    for (i=0; i<needs.length; i++){
      ...
      if ( !context.hasOwnProperty(need) ){
        ...
        if ( context.hasOwnProperty(need) ){
          // when the context was set in scope(), replace exports in cache
          require.cache[ require.resolve(need) ].exports = context[need];
        } else {
